...
class YAMLStoryReader(StoryReader):
    ...
    def _parse_plain_story_user_utterance(self, step: Dict[Text, Any]) -> None:
        ...
        raw_user_utterances = step.get(KEY_PLAIN_STORY_USER_UTTERANCE, "").strip()
        ...
        if self.use_e2e:
            ...
            utterances = YAMLStoryReader._parse_raw_user_utterances(
                raw_user_utterances, entities
            )
            self.current_step_builder.add_user_messages(utterances)

    @staticmethod
    def _parse_raw_user_utterances(
        user_utterances: Text, entities: List
    ) -> List[UserUttered]:
        ...
        result = []
        for utterance in [el.strip() for el in user_utterances.split(KEYWORD_OR)]:
            if not utterance.startswith(INTENT_MESSAGE_PREFIX):
                raise_warning(
                    f'User intent "{utterance}" should start with '
                    f'"{INTENT_MESSAGE_PREFIX}"',
                    docs=DOCS_URL_STORIES,
                )
            else:
                utterance = utterance[1:]
            intent = {"name": utterance, "confidence": 1.0}
            result.append(UserUttered(utterance, intent, entities_list))
