...
class SimplePolicyEnsemble(PolicyEnsemble):
    ...
    def _best_policy_prediction(
        ...
        predictions = {
            f"policy_{i}_{type(p).__name__}": self._get_prediction(
                p, tracker, domain, interpreter, p.priority
            )
    ...
    def _get_prediction(
        ...
        if (
            len(common_utils.arguments_of(policy.predict_action_probabilities))
            > number_of_arguments_in_rasa_1_0
        ):
        ...
        return Prediction(probabilities, priority)
